...
NoArvB* insereArvoreB(NoArvB *raiz, int diretorio) { /* Fazemos efetivamente a inserção nessa função. A outra é só uma parte. */
...
void merge(NoArvB *atual, int pos){
    ...
    strcpy(filho->chave[T - 1], atual->chave[pos]); /* Chave do pai desce ao filho (1) */

    for(int j = 0; j < posterior->n; j++){ /* Copiar para o filho os valores do posterior (2) */
        strcpy(filho->chave[j + T], posterior->chave[j]);
    }
